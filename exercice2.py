...
top_color = {}
for person in survey:
    if(person["color_prefered"] in top_color.keys()):
        top_color[person["color_prefered"]] += 1
    else:
        top_color[person["color_prefered"]] = 1
sorted_color = sorted(top_color.items(), key=lambda item:item[1], reverse=True)
print(sorted_color)
...
for person in survey:
    day_of_birth = person["date_of_birth"][0:2]
    month_of_birth = person["date_of_birth"][3:5]
    year_of_birth = person["date_of_birth"][6:10]
    ...
    if month_of_birth == today[5:7] and int(day_of_birth) >= int(today[-2:]):
        ...
        print(f"Le {day_of_birth} {months[int(month_of_birth)-1]} {person['firstname']} {person['lastname']} fêtera ses {age} ans")
